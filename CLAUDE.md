...
| 3 | Search view (filter tab pills + grouped result lists, per `design/rendered/new-search.html`) | Next |
| 4 | Playlist detail view (read Plex playlists) | Not started |
...
Git log is the authoritative record of what shipped. `php artisan test` should be green (47 tests as of end of M2, including one Playwright browser test).
...
## Gotchas (hard won from M1/M2 — read before touching these areas)
...
- **`#[Computed]` methods do NOT autowire type hinted parameters** in this Livewire version. Resolve services via `app(SomeService::class)` inside the computed. Action and lifecycle methods (`mount`, `boot`, `wire:click` handlers) DO autowire params, so prefer param injection there. (A `boot(PlexClient $plex)` + `protected PlexClient $plex` pattern would be cleaner than repeated `app()` calls in computeds; the library view currently uses `app()` everywhere, fine to refactor when touching it.)
- **Plex bulk listings don't include child counts or durations.** `/library/sections/{id}/all?type=8` (artists) has no `childCount`. `/library/metadata/{artistId}/children` (albums) has no `leafCount` or `duration`. So artist rows don't show "{N} albums" and album rows don't show "{N} tracks" (those were dropped). The album header computes its "{N} songs, {duration}" from the loaded tracklist (`$this->tracks->count()` / `->sum('durationMs')`) instead. **For M3:** Plex's `/hubs/search` endpoint MAY return richer metadata (the new search design shows "5 albums · 62 tracks" on artist rows and durations on album rows) — verify against the live API when building it; if it doesn't, drop those bits the same way.
