...
| 7 | Polish (keyboard shortcuts, transitions, error toasts, compact/comfortable density wiring, scrobble-to-Plex, formatMs hour handling, etc.) | Next |
...
Git log is the authoritative record of what shipped. `php artisan test` should be green (190 tests, including 17 Playwright browser tests).
...
- **`play-track` event payload is `queue` / `index` / `shuffle`.** The `queue` field is an array of objects with shape `{id, url, title, artist, artwork, albumId, artistId}` (build each via `PlexClient::queueItem(Track, ?artworkOverride)` — don't hand-roll the array); `index` is the int position to start from; `shuffle` is bool. Every view that plays a track (library album tracklist, playlist detail, search Songs group, recently-played list, sidebar playlist) must dispatch the whole relevant list, not a single track. The queue is client-side only (Alpine `audioPlayer()` scope), sent once as the event payload, so it never appears in the Livewire snapshot. `streamUrl()`/`thumbUrl()` are pure string building, so resolving a whole list server-side up front is cheap. The now-playing strip uses `albumId`/`artistId` from `current` to `Livewire.navigate('/?artist=…&album=…')` on click (`audioPlayer().goToAlbum()`/`goToArtist()`). One syntax trap: `@error="..."` on the `<audio>` element is parsed as the Blade `@error` directive (form validation helper), so the native error event must be bound with `x-on:error="onTrackError()"` instead.

## Test layout

- `tests/Unit/` — `PlexClientTest` (Http::fake), `PlexCacheTest`, `PlexDtoTest`, `AppSettingTest`. Fast, no Plex.
- `tests/Feature/` — `RoutesTest`, `SettingTest`, `PlayerComponentTest`, `LibraryViewTest`, `ServerChipTest`, `SearchViewTest`, `TopbarTest`, `PlaylistDetailViewTest`, `SidebarTest`, `FolderTest`, `SettingsViewTest`. Use mocked `PlexClient` (and `Livewire::withQueryParams([...])->test(...)` for `#[Url]` props). Note `Livewire::test('sidebar')` (and `Livewire::test('pages::settings')`, which renders the layout including the sidebar) also renders the nested `<livewire:server-chip />`, so the `PlexClient` mock must stub `ping()` too.
- `tests/Browser/` — `LibraryDrillDownTest`, `SearchTest`, `PlaylistDetailTest`, `SidebarFoldersTest`, `PlayerQueueTest`, `SettingsTest` (Playwright, hit live Plex). `tests/Browser/Screenshots/` is gitignored.
