...
**Reorder tracks within a playlist (shipped 2026-05-12, branch `feat/playlist-reorder`).** Spec: `docs/superpowers/specs/2026-05-12-plextune-playlist-reorder-design.md`. Plan: `docs/superpowers/plans/2026-05-12-plextune-playlist-reorder.md`. Drag a track row up or down in the playlist-detail tracklist (both density variants) to change its position, persisted to Plex. Plex's `GET /playlists/{id}/items` rows carry `playlistItemID` (the per-playlist slot id, distinct from the track's `ratingKey`) — added to the `Track` DTO as a nullable `playlistItemId` (only `playlistTracks()` rows have it; library/search/recently-played rows are null). Adding it required `php artisan cache:clear` (the documented stale-cached-DTO gotcha). `PlexClient::moveTrack(playlistId, playlistItemId, ?afterPlaylistItemId)` does `PUT /playlists/{id}/items/{itemId}/move?after=...` (no `after` = move to front), busting only `plex:playlist:{id}:items`. The `pages::playlist-detail` component gained a `moveTrack(draggedItemId, targetItemId, position): bool` action: "drop before target" is expressed as `move?after={the item currently above the target}` (the first-row case = no `after`); no-op short-circuits for self-drops and already-correct positions; a Plex outage dispatches a `notify` error toast and returns `false` (the DnD-write `bool`/notify pattern, like the sidebar's playlist mutations); success `unset($this->tracks)` to re-fetch. The tracklist scroll container's `x-data` gained a native-HTML5-DnD scope (`draggedId`/`overId`/`overPos`/`flash` + `onDragOver`/`onDrop`/`flashRow`); both row variants got `@dragstart` (still sets `plextune/track` so the existing "drag onto a sidebar playlist" feature is unaffected, plus `plextune/playlist-item` + `draggedId`), `@dragend`/`@dragover.prevent`/`@dragleave` (the `dragleave` guard checks `currentTarget.contains(relatedTarget)` so the line doesn't flicker on child hover)/`@drop.prevent`, and a `:class` with `.drop-before`/`.drop-after` (two `inset box-shadow` utilities in `@layer base`, no layout shift) plus the ~700ms green/red ring flash. The library album tracklist and search rows were intentionally left as plain `ondragstart` (no reorder). Tests: extended `tests/Unit/PlexDtoTest.php` (+2), `tests/Unit/PlexClientTest.php` (+3), `tests/Feature/PlaylistDetailViewTest.php` (+6), `tests/Browser/PlaylistDetailTest.php` (+1, calls the `moveTrack` action via `script()`/`window.Livewire` and round-trips the live playlist's order).

**Reorder playlists and folders in the sidebar (shipped 2026-05-13, branch `feat/sidebar-reorder`).** Spec: `docs/superpowers/specs/2026-05-12-plextune-sidebar-reorder-design.md`. Plan: `docs/superpowers/plans/2026-05-12-plextune-sidebar-reorder.md`. Drag root playlists to reorder, drag within a folder, drop into a folder header (lands at the top, no auto-expand), drop on the "Other" header (lands at root end), drag folder headers to reorder folders. All ordering is app-side (Eloquent), no Plex calls. `folder_playlists.folder_id` is now nullable (migration `2026_05_12_000001_make_folder_playlists_folder_id_nullable.php`): a row with `folder_id` null = "this playlist is pinned at root at this position"; no row = unplaced root playlist (sorts after placed ones in Plex order). The FK is `nullOnDelete`, so deleting a folder now moves its playlists to root (was: cascade-delete the rows). `FolderPlaylist` keeps its name even though `folder_id` can be null (documented misnomer; renaming would churn a lot of M5 code). The sidebar SFC gained: `rootPlacements()` `#[Computed]`; a `protected rootOrderedPlexIds(allPlexIds, filedIds): list<string>` helper used both by the render and the move actions (placed-by-position then unplaced-by-Plex-order); a `movePlaylist(draggedPlaylistId, ?targetFolderId, ?targetPlaylistId, position): void` Livewire action with `placeInRoot`/`placeInFolder` branches (`targetPlaylistId === null` = "front of the destination container"; into-folder bumps existing members; cross-container renumbers the source via `renumberContainer(?int)`; out-to-root materializes a row for every current root playlist — a one-time densification); a `moveFolder(draggedFolderId, targetFolderId, position): void` action that reuses the shared `insertRelative` helper (folder ids cast to/from strings) and renumbers folders 0..N. Both actions are pure DB writes — no `notify` toast / `bool` return like the Plex-backed actions. DnD wiring: the playlists-card `x-data` scope gained `draggingPlaylist`/`draggedPlaylistId`/`draggingFolder`/`draggedFolderId`/`overId`/`overPos`, plus helpers `rowDragOver`/`folderDragOver`/`onRowDrop`/`onFolderHeaderDrop`/`dropOnOtherHeader`; the window `dragstart` listener now sniffs `plextune/playlist` and `plextune/folder` in addition to `plextune/track`. The row partial gained a `$folderId` param and a unified `@drop` handler that branches on `dataTransfer.getData(...)` (`plextune/playlist` → reorder; `plextune/track`/`plextune/album` → existing add behavior). Folder headers got `draggable="true"` + `@dragstart` (`plextune/folder` + Alpine state); `wire:click="toggleFolder"` and `draggable="true"` coexist on the same element (a click toggles, a drag reorders), matching the playlist-track-row pattern. Insertion line reuses the `.drop-before`/`.drop-after` `inset box-shadow` utilities from `feat/playlist-reorder` (no CSS change). Context-menu "Move to {folder}" / "Remove from folder" entries kept the existing `movePlaylistToFolder($id, ?int)` action (append-on-add, delete-row-on-remove) — coarse append/remove semantics distinct from the positional drag UX. Tests: `tests/Feature/FolderTest.php` (updated cascade test → "moves to root"), `tests/Feature/SidebarTest.php` (updated delete-folder test + 12 new tests for `movePlaylist`/`moveFolder`/ordering), `tests/Browser/SidebarFoldersTest.php` (+1 reorder test that drives `$wire.movePlaylist(...)` via `script()` and round-trips the order; the file's `afterEach` now explicitly clears `folder_playlists` since the FK no longer cascades).

**AI playlist generator (shipped 2026-05-17, branch `feat/ai-playlist-generator`).** Spec: `docs/superpowers/specs/2026-05-17-plextune-ai-playlist-generator-design.md`. Plan: `docs/superpowers/plans/2026-05-17-plextune-ai-playlist-generator.md`. New `/generate` page (`pages/⚡generate.blade.php`) drives a `laravel/ai` v0 Agent (`App\Ai\Agents\PlaylistGeneratorAgent`) that has tools wrapping `PlexClient` (`ListTaxonomy`, `FindTracks`, `PopularTracksForArtist`, `SimilarArtists`, `SearchArtists`, `SearchAlbums` in `app/Ai/Tools/`) plus a terminal `ProposePlaylist` tool that writes to a new `ai_playlist_proposals` table (UUID `conversation_id`, JSON `payload`, `status` of `pending`/`accepted`/`discarded`, optional `plex_playlist_id`). The agent's system prompt embeds a live snapshot of the library's real Genre/Style/Mood names (via `App\Ai\Support\LibraryTaxonomy`) so the model only filters by tags that exist. Conversations persist via the SDK's `RemembersConversations` trait against a single seeded `plexify@local` user. The page pre-creates the conversation via `Laravel\Ai\Contracts\ConversationStore::storeConversation()` before the first `prompt()` so tools can see the id (see Gotchas). Accept creates the playlist on Plex (seed track + add the rest), marks the proposal `accepted`, and dispatches `playlist-created`; Refine pre-fills the input; Discard marks the proposal `discarded`. Provider/model are `.env`-driven (`AI_PROVIDER=openai`, `AI_MODEL=gpt-4o`). New `PlexClient` methods: `genres()/styles()/moods()` (taxonomy lookups, cached 24h), `findTracks($filters)` (combined style/mood/genre/year/ratingKeys filter, uncached), `popularTracksForArtist($id)`, `similarArtists($id)`. New: sidebar "Generate" link above "Your Library" using `x-lucide-sparkles`. Tests: 16 new files / +30 tests across `tests/Unit/Ai/*`, `tests/Feature/GeneratePageTest.php`, `tests/Browser/GenerateTest.php`; full suite 329+ green.

**Music library picker (shipped 2026-07-16, branch `feat/library-picker`).** Spec: `docs/superpowers/specs/2026-07-16-plextune-library-picker-design.md`. Plan: `docs/superpowers/plans/2026-07-16-plextune-library-picker.md`. Root cause: the app used to derive its Plex music library from the first `type=artist` entry in `/library/sections` and cache that choice for 24h, which silently switched the app between libraries whenever Plex reordered its response (this actually happened, moving the app from "Music" to "Classical"). The library is now a persisted choice with a deterministic fallback, not a derivation from response order. New `MusicSection` DTO (`{int $id, string $title}`). `PlexClient::musicSections()` lists artist-type sections (cached, server-wide, key `music_sections`) and `musicSectionTitle()` resolves the active section's title. `musicSectionId()` was rewritten: it reads `AppSetting::musicSectionId()` and, if that saved id no longer matches a real section, falls back to the section with the lowest key; it is deliberately NOT cached. `AppSetting::musicSectionId()`/`setMusicSectionId()` wrap the new `music_section_id` setting (nullable raw value vs. the non-null effective value PlexClient resolves). Cache keys that vary by library are now section-namespaced with an `s{id}:` prefix via a new `sectionKey()` helper (`artists`, `recently_added`, `recently_played`, `favorites`, `taxonomy`); ratingKey-keyed entries (`albums:{artistId}`, `tracks:{albumId}`, `artist:{id}:popular`) and server-wide entries (`playlists`, `base_url`, `machine_identifier`, `music_sections`) are not namespaced, since they are already unambiguous or intentionally shared. Search scoping: `/hubs/search` ignores any section parameter and searches the whole server, so `searchAll()` now filters hub rows on `librarySectionID` in PHP after the fetch; playlist hub rows carry no `librarySectionID` and stay unfiltered (Plex playlists aren't section-scoped) so they don't vanish from search results. The Settings page gained a "Music library" `<select>` (`wire:model.live="musicSectionId"`) that renders whenever `musicSections()` is non-empty (i.e. whenever the server has at least one artist-type section, not only when there is more than one), calling `updatedMusicSectionId()` to persist the choice after validating it against the live section list; on a valid switch it also dispatches a `library-changed` browser event so the sidebar server chip's `#[On('library-changed')]` handler can re-ping and refresh its subtitle without waiting for a navigation. The sidebar server chip now shows the effective library name in its subtitle, e.g. "Music · Direct connection". `config/cache.php`'s `serializable_classes` allowlist gained `MusicSection::class` (see the `serializable_classes` gotcha below); this did not require a `php artisan cache:clear` for correctness, since `MusicSection` collections are cached under the brand-new `music_sections` key rather than an existing one, so there is no stale-cached-DTO hazard. `cache:clear` is still worth running once after deploying this branch, purely as hygiene: it evicts the now-orphaned un-namespaced `plex:artists` / `plex:recently_*` / `plex:favorites:*` / `plex:taxonomy:*` entries left over from before section-namespacing, which would otherwise sit unread (holding the old library's data) for up to 24h (7 days for `tracks:*`). Tests: extended `tests/Unit/PlexDtoTest.php` (`MusicSection`), `tests/Unit/PlexClientTest.php` (`musicSections()`, the rewritten `musicSectionId()`, section-namespaced cache keys, search scoping), `tests/Unit/AppSettingTest.php`, `tests/Feature/ServerChipTest.php`, `tests/Feature/SettingsViewTest.php`, `tests/Feature/SidebarTest.php`, `tests/Feature/RoutesTest.php`; two new `tests/Browser/SettingsTest.php` cases (switching the library round-trips through `AppSetting::musicSectionId()`, and the sidebar chip reflects the active library name).

Git log is the authoritative record of what shipped. `php artisan test` should be green (329+ tests, including 24 Playwright browser tests).
...
- **`config/cache.php` has a `serializable_classes` allowlist** (`Collection::class`, `Artist::class`, `Album::class`, `Track::class`, `Playlist::class`, `MusicSection::class`) because `PlexCache` stores DTO collections and Laravel 11.41+ blocks unserializing cached objects by default. A cleaner long term fix is to have `PlexCache` store raw arrays and rehydrate DTOs on read, then drop the broad `Collection::class` entry; not done yet. (M4 added `Playlist::class` when `playlists()`/`playlistTracks()` started caching; the library picker branch added `MusicSection::class` when `musicSections()` started caching.) **Adding a property to a cached DTO requires `php artisan cache:clear`** (the cache driver here is `database`, so old serialized DTOs persist across deploys; PHP's `unserialize` doesn't run the constructor, so a newly-added typed property comes back uninitialized and the first access throws "must not be accessed before initialization"). The Settings page's "Resync metadata" button (M6) does the same thing via `PlexCache::flushAll()`.
...
- **laravel/ai's `RemembersConversations::currentConversation()` returns null on a fresh conversation until the post-LLM middleware creates one** (`vendor/laravel/ai/src/Middleware/RememberConversation.php`'s `then()` callback). Because our `ProposePlaylist` tool needs the conversation_id at construct time (its constructor signature is `string $conversationId`), the `/generate` page MUST pre-create the conversation via `app(\Laravel\Ai\Contracts\ConversationStore::class)->storeConversation($user->id, $title)` BEFORE calling `->continue($id, as: $user)->prompt($text)`. The agent's `tools()` method throws a `RuntimeException` if `currentConversation()` is null, to catch any caller that forgets.
- **Livewire 4 SFC files use `<?php ... ?>` (raw PHP) not `@php ... @endphp`** for the component class declaration. Livewire's SFC detector regex matches `<?php.*new\s+.*class`, so `@php` fails the check and the page renders as a plain Blade file. (Bit T19 of the AI generator until corrected.)
- **The `⚡` filename prefix is Livewire-only.** Plain Blade anonymous components (e.g. `generate-message.blade.php`, `generate-proposal-card.blade.php`) must NOT use it, or the component compiler errors with "Unable to locate a class or view for component".
- **In feature tests, `env('FOO')` does NOT see values set via `putenv('FOO=...')`** because Laravel's `env()` reads from the Dotenv-loaded `$_ENV` baked at boot. Use `getenv('FOO')` in code that you want to drive from `putenv()` in tests (or set the env var before booting Laravel). The `/generate` page reads `getenv('OPENAI_API_KEY')` for exactly this reason.
- **The laravel/ai SDK's conversation tables are `agent_conversations` and `agent_conversation_messages`** (NOT `ai_conversations`), and conversation IDs are UUID strings (created via `Str::uuid7()`), not auto-increment integers. Our `ai_playlist_proposals.conversation_id` is a `string` column.
- **The music library is a persisted choice, not a derivation.** `PlexClient::musicSectionId()`
  ...
  ratingKey-keyed entries (`albums:{artistId}`, `tracks:{albumId}`, `artist:{id}:popular`,
  `playlist:{playlistId}:items`) and server-wide entries (`playlists`, `base_url`,
  `machine_identifier`, `music_sections`, `owned_resource`) do not. `rateTrack()`'s
  `forgetFavoritesCache()` deliberately busts EVERY section's `favorites:*` entry (it scans
  `plex:_index` for keys containing `:favorites:`), not only the currently-selected section's:
  `rateTrack()` takes a bare ratingKey with no section context, and a track can be rated from the
  now-playing strip after a library switch, so the rated track is not necessarily in the
  currently-selected section. This is the easiest of these caches to accidentally re-scope to "just
  the current section" during a refactor, which would silently stop invalidating favorites rated
...
- **A browser test that polls a DOM value its own script just forced proves nothing about the
  Livewire round-trip.** Setting `select.value = X` then polling for `select.value === X` returns
  true on the first iteration whether or not the server was ever reached, so a following DB
  assertion races an in-flight request and fails intermittently. Drive Livewire directly instead:
  `const comp = window.Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id'));
  await comp.set('prop', value)` (or `.call(...)`), which resolves only after the request
  completes. This pattern is used in `tests/Browser/PlaylistDetailTest.php`,
  `tests/Browser/SidebarFoldersTest.php`, and `tests/Browser/SettingsTest.php`.

## Test layout

- `tests/Unit/` — `PlexClientTest` (Http::fake), `PlexCacheTest`, `PlexDtoTest`, `AppSettingTest`, `DurationTest`. Fast, no Plex.
- `tests/Feature/` — `RoutesTest`, `SettingTest`, `PlayerComponentTest`, `LibraryViewTest`, `ServerChipTest`, `SearchComponentTest` (replaced `SearchViewTest`), `TopbarTest`, `PlaylistDetailViewTest`, `SidebarTest`, `FolderTest`, `SettingsViewTest`, `ToastsTest`. Use mocked `PlexClient` (and `Livewire::withQueryParams([...])->test(...)` for `#[Url]` props). Note `Livewire::test('sidebar')` renders the nested `<livewire:server-chip />`, so the `PlexClient` mock must stub `ping()` too. `Livewire::test('pages::X')` renders that component in isolation, without the layout, so it does NOT render the sidebar or the server chip; only a full-page HTTP request or `Livewire::test('sidebar')` does. The layout now also renders `<livewire:search />` — tests that mock `PlexClient` should stub `searchAll` when rendering a full layout with `?q=` set.
