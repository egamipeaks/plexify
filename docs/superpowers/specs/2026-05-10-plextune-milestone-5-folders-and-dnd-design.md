...
Turn the flat sidebar playlist list into app-side folders, and add native HTML5 drag-and-drop: drag playlists between folders, drag a track row or an album cover onto a playlist to add it. Folders are pure Eloquent (Plex has no server-side folders). Also bring the sidebar's playlist write surface up to full CRUD against Plex: create (lazily, via a draft row), rename (write-through), delete. Context menus on folders and playlist rows. Filter input wired client-side.

End state: you can organize your Plex playlists into collapsible folders that survive a browser reset, drag songs/albums onto playlists, and create/rename/delete playlists from the sidebar.
...
| "New playlist" button | Draft-row flow (Plex can't create an empty playlist) | Clicking `+` adds a local draft row with an inline name input; the real Plex playlist is created (`POST /playlists` with the first item as the seed `uri`) on the first track drop onto that draft. A draft with no name / no drop is discarded on blur. |
...
- public `$renamingFolderId` / `$renamingPlaylistId` / `$draftPlaylistName` (transient inline-edit state).
...
4. The **draft playlist row** (when one is being created) renders at the bottom of the root list with an autofocused name input.
...
- **New playlist** (`+`) button → `createDraftPlaylist()` (adds the draft row, focuses its input).
...
- `createDraftPlaylist(): void` / `cancelDraftPlaylist(): void` — manage the transient draft row (no DB / no Plex call yet).
- `dropOnDraftPlaylist(string $name, string $trackId): void` — `PlexClient::createPlaylist($name ?: 'New Playlist', $trackId)`; on success clear the draft (the real playlist now shows because `plex:playlists` was busted).
...
|---|---|---|
| Track row — **library** tracklist (`pages/⚡library.blade.php`), **playlist-detail** tracklist (`pages/⚡playlist-detail.blade.php`), **search** song rows (`pages/⚡search.blade.php`) | `plextune/track` → track rating key | playlist row (real or draft) |
| Album cover — **library** album header, both expanded and collapsed states | `plextune/album` → album rating key | playlist row (real only — a draft seeds from a single track) |
...
- `@dragover.prevent` (also toggles a `bg-accent/15 ring-1 ring-accent/40` highlight on the target), `@dragleave` clears it, `@drop` clears it and dispatches:
  ...
  - draft playlist row receiving `plextune/track` → `$wire.dropOnDraftPlaylist(draftName, trackId)`.
...
- After a `$wire` call resolves: flash the target row green (success) or red (the action threw / Livewire error) for ~600ms, then return to normal. The sidebar re-renders from the busted cache so playlist counts update.
...
- A draft playlist whose creation fails stays as a draft row (red flash); the user can retry by dropping again or cancel it.
...
- `createDraftPlaylist` shows the draft row; `dropOnDraftPlaylist` calls `PlexClient::createPlaylist` and clears the draft.
...
- `resources/views/components/⚡sidebar.blade.php` — folders, DnD targets, context menus, filter input, draft-playlist flow, the new actions.
